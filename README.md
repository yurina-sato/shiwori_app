...
- 旅行後は写真をメンバーが各々まとめて送り合うので、写真フォルダが溢れてしまう  
  →日付やイベント毎に写真をまとめたい
...
### しおり機能 
旅行毎の大枠となるページを作成します。  
旅行毎のトップページとなり、旅行の概要、メンバー、スケジュール、リストを表示します。

### スケジュール機能
旅行の1日毎のスケジュールを管理できるページを作成します。  
...
### 写真投稿機能
イベント毎に写真をアップロードできる機能です。  
時系列で並ぶイベントに写真も紐付くことで、旅行後も振り返りやすくなると考えています。

### リスト管理機能
旅行毎にリスト形式のメモを作成する機能です。  
計画時のToDoや、買い物、持ち物、お金の建て替え状況などをリストの形で管理することができます。

### アイテム機能
リストに投稿する一つひとつの項目を作成します。  
実行項目をクリックすることで、色を薄くして他アイテムと区別できる仕様を想定しています。
...
トップページから旅行毎にしおりを作成することができます。<br>
...
また、スケジュール内にイベントを作成すると、スケジュール詳細ページに時系列で表示されます。  
...
## 追加実装予定の機能
### チャットルーム機能
しおり毎にメンバーとやり取りができるチャットルームを作成します。  
普段使うメッセージアプリのグループとは他に、しおりの中に旅行専用のチャットルームがあることで、情報が埋もれずスムーズな共有ができると考えています。  
しおりの更新履歴や旅先情報のURLを投稿するといった、掲示板のようなイメージを想定しています。
...
学生サークルや社員旅行のような大人数旅行の場合に、編集権限のないメンバーへもしおり共有をできるように実装したいと考えています。

## 改良したい課題
- 画像プレビューの改良(プレビュー上で画像の差し替えや削除を可能に)
- メンバー選択フォームの改良
- しおりへ参加するユーザーの招待制、主催者の設定
- 実行済リスト項目処理のJavaScriptを改良(チェックマークの挿入)
